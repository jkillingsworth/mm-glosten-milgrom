...
let renderPrices path data =
    ...
    series.StrokeThickness <- 1.0
    data
    |> Array.mapi (fun i (value, (_, _, bid), (_, _, ask), p) -> DataPoint(float i, bid))
    |> Array.iter series.Points.Add
    data
    |> Array.mapi (fun i (value, (_, _, bid), (_, _, ask), p) -> DataPoint(float i, ask))
...
let renderBelief path data =
    ...
    series.StrokeThickness <- 1.0
    p
    |> Array.mapi (fun i x -> DataPoint(Compute.vMin + float i * Compute.vInc, x * adjustment))
    |> Array.iter series.Points.Add
    model.Series.Add(series)
    ...
    series.Title <- "MM price"
    series.StrokeThickness <- 1.0
    series.Points.Add(DataPoint(vBid, p.[iBid] * adjustment))
    series.Points.Add(DataPoint(vAsk, p.[iAsk] * adjustment))
    model.Series.Add(series)
